 ...
 * @FilePath: ql-gateway/internal/middleware/jwt.go
 */
...
func (mw *GinJWTMiddleware) MiddlewareInit() error {
	...
	if conf.C.Login.Timeout > 0 {
		// TODO: token过期时长
		mw.Timeout = time.Duration(conf.C.Login.Timeout) * time.Second
	}
	...
	if mw.LoginResponse == nil {
		mw.LoginResponse = func(c *gin.Context, code int, token string, expire time.Time) {
			encoding.Ok(c, gin.H{
				"code":   http.StatusOK,
				"token":  token,
				"expire": expire.Format(constant.Layout),
			})
		}
	}
...
func (mw *GinJWTMiddleware) LoginHandler(c *gin.Context) {
	...
	dataType := data.(map[string]interface{})
	var u model.Account
	if err = mapstructure.Decode(dataType["user"], &u); err != nil {
		log.Error(zap.Any("failed to parse interface data to struct", zap.Error(err)))
		return
	}

	result := protocol.LoginResp{
		Uid:        strconv.FormatUint(u.Uid, 10),
		Account:    u.Account,
		Username:   u.Username,
		Avatar:     u.Avatar,
		Time:       time.Now().Format(constant.Layout),
		Token:      tokenString,
		ExpireTime: expire.Format(constant.Layout),
	}
