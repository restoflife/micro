 ...
 * @IDE: GoLand
 * @Date: 2022-04-22 16:15
 * @LastEditors: Administrator
 * @LastEditTime: 2022-04-22 16:15
 * @FilePath: ql-gateway/internal/ihttp/client.go
 */
...
func NewClient() (client *Client) {
	client = &Client{
		...
		requestType:   constant.TypeJSON,
		unmarshalType: string(constant.TypeJSON),
	}
	return client
}
...
func (c *Client) Type(typeStr constant.RequestType) (client *Client) {
	if _, ok := constant.Types[typeStr]; ok {
...
func (c *Client) EndBytes(ctx context.Context) (res *http.Response, bs []byte, err error) {
	...
	// multipart-form-data
	if c.requestType == constant.TypeMultipartFormData {
		body = &bytes.Buffer{}
	...
	reqFunc := func() (err error) {
		...
		case http.MethodGet:
			...
			case constant.TypeJSON:
				c.ContentType = constant.Types[constant.TypeJSON]
			case constant.TypeForm, constant.TypeFormData, constant.TypeUrlencoded:
				c.ContentType = constant.Types[constant.TypeForm]
			case constant.TypeMultipartFormData:
				c.ContentType = bw.FormDataContentType()
			case constant.TypeXML:
				c.ContentType = constant.Types[constant.TypeXML]
				c.unmarshalType = string(constant.TypeXML)
			default:
		...
		case http.MethodPost, http.MethodPut, http.MethodDelete, http.MethodPatch:
			...
			case constant.TypeMultipartFormData:
				for k, v := range c.multipartBodyMap {
					...
					} else if ss := utils.ConvertToString(v); ss != "" {
...
func (c *Client) SendStruct(v interface{}) (client *Client) {
	...
	case constant.TypeJSON:
		c.jsonByte = bs
	case constant.TypeXML, constant.TypeUrlencoded, constant.TypeForm, constant.TypeFormData:
...
func FormatURLParam(body map[string]interface{}) (urlParam string) {
	...
	for _, k := range keys {
		v, ok := body[k].(string)
		if !ok {
			v = utils.ConvertToString(body[k])
		}
		if v != "" {
